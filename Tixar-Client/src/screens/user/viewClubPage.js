...
export default ViewClubPage = ({ route, navigation }) => {
  const { clubName, artistDescription, clubId, profileId, imageUrl, points } =
    route.params;
  const { token } = useContext(AuthContext);
  const { colors } = useContext(ColorContext);

  const handleDeletePress = () => {
    fetch(`http://vf.tixar.sg/api/profile/${profileId}`, {
      method: "DELETE",
      credentials: "include",
    ...
    >
      <View style={{ backgroundColor: colors.background }}>
        ...
        >
          ...
          {profileId === null && (
            <View
              style={{ width: "50%", overflow: "hidden", borderRadius: 15 }}
            >
              <Button
