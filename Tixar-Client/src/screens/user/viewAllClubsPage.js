...
export default ViewAllClubsPage = ({ route, navigation }) => {
  ...
  const getProfiles = async () => {
    await fetch("http://vf.tixar.sg/api/profiles", {
      method: "GET",
      credentials: "include",
    ...
    await fetch("http://vf.tixar.sg/api/clubs", {
      method: "GET",
      credentials: "include",
      headers: { Authorization: "Bearer " + token },
    })
  ...
  useEffect(() => {
    ...
    setClubs(
      allClubs.filter((club) => club.name.toLowerCase().includes(query))
    );
  }, [searchText]);
            ...
            <View>
              ...
              <ClubsCard
                ...
                onPressFunction={() => {
                  ...
                  let points = null;
                  if (mem) {
                    const prof = allProfiles
                      .filter((prof) => prof.club._id === item._id)
                      .pop();
                    pId = prof._id;
                    points = prof.points;
                  }
