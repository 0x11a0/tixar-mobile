...
export default ConfirmCodePage = ({ route, navigation }) => {
  ...
  const [rotateRightPercent, setRotateRightPercent] = useState(
    new Animated.Value(0)
  );
  const [circleOpacity, setCircleOpacity] = useState(new Animated.Value(1));
  const [rotateLeftPercent, setRotateLeftPercent] = useState(
    new Animated.Value(0)
  );
    ...
    <View style={styles.container}>
      <View style={styles.upperBlock}>
        ...
        <View style={styles.fieldBox}>
          <Text style={styles.fieldText}>{route.params.code}</Text>
        </View>
        <View style={{ height: 20 }} />
        <View style={styles.fieldBox}>
          <Text style={styles.fieldText}>{route.params.points}</Text>
        </View>
        <View style={{ height: 20 }} />
        <View style={styles.fieldBox}>
          <Text style={styles.fieldText}>{route.params.expiryDate}</Text>
        </View>
      </View>
